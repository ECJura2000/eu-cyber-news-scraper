...
from .config import DEFAULT_DAYS, DEFAULT_OUTPUT_DIR, DEFAULT_TIMEOUT, DEFAULT_TIMEZONE, DEFAULT_WORKERS, load_sources
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--days", type=int, default=DEFAULT_DAYS, help=f"回推日數，預設 {DEFAULT_DAYS} 天。")
    parser.add_argument("--since", help="起始日 YYYY-MM-DD；會覆蓋 --days。")
    parser.add_argument("--until", help="結束日 YYYY-MM-DD；包含該日。")
...
def main() -> None:
    ...
    since, until = _date_range(args.since, args.until, args.days)
    output = Path(args.output) if args.output else _default_output(since, until)
    ...
    try:
        _run_pipeline(args, selected, since, until, output, run_id)
    finally:
        release_run_lock(lock_path, run_id)


def _run_pipeline(args, selected, since, until, output: Path, run_id: str) -> None:
    ...
    articles = dedupe_articles([article for result in results for article in result.articles])
    if args.topic:
    ...
    statuses = assess_and_record_health(
        statuses,
        selected,
        output.parent / ".source-health.json",
        run_id=run_id,
    ...
    degraded = [status.source_id for status in statuses if status.critical and not status.success]
...
def _date_range(since_value: str | None, until_value: str | None, days: int) -> tuple[datetime, datetime]:
    ...
    try:
        start_date = date.fromisoformat(since_value) if since_value else today - timedelta(days=days)
        end_date = date.fromisoformat(until_value) if until_value else today
    except ValueError as exc:
        raise SystemExit("[error] 日期格式必須為 YYYY-MM-DD，且必須是有效日期。") from exc
    if start_date > end_date:
        raise SystemExit("[error] 起始日不得晚於結束日。")
